...
class Dupak(models.Model):
    ...
    @api.multi
    def action_state_review(self):
        self.state = 'review'

    @api.multi
    def action_state_itb(self):
        self.state = 'itb'

    @api.multi
    def action_state_dikti(self):
        self.state = 'dikti'
    ...
    def update_dupak(self):
        ...
        if publications:
            ...
            for item in publications_shared:
                ...
                semester = semesters.filtered(lambda x: x.start <= item.publication_id.day and x.finish >= item.publication_id.day)
                #period = semester.name
                period = datetime.strptime(item.publication_id.day, '%Y-%m-%d').strftime('%d %B %Y') if item.publication_id.day else ''
                scopus = item.publication_id.scopus if item.publication_id.scopus else ''
