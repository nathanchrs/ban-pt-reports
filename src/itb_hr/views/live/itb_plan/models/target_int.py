...
class Target_Int(models.Model):
	_name = 'itb.plan_target_int'
	...
	plan_line_id = fields.Many2one('itb.plan_line_int',domain="[('plan_int_id','=',plan_int_id)]",ondelete='cascade',required=True,index=True)
	indicator_id = fields.Many2one('itb.plan_indicator',ondelete='cascade',required=True,index=True)
	plan_int_id = fields.Many2one(related='plan_line_id.plan_int_id',store='True',index=True,domain="[('state','=','validate')]")
	...
	@api.one
	@api.depends('plan_int_id', 'actual', 'indicator_id')
