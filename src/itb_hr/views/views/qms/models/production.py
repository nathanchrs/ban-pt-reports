...
class production(models.Model):
    _name = 'qms.production'

    site_id = fields.Many2one('qms.site',string='Site')
    product_id = fields.Many2one('product.product',domain="[('sale_ok','=',True)]",string='Product',required=True)
    coa_ids = fields.One2many('qms.coa','production_id',string='Certificate of Analysis')
    coa = fields.Boolean()
    ...
    def create_coa(self):
        ...
        if parameters:
            for x in parameters: 
                output = (0,0,{'source':'production','production_id': self.id,'product_id':self.product_id.id,'date':self.date,'reference':self.name,'parameter_id':x.id,'result':0})
                coas.append(output)

            self.write({'coa_ids': [(2,coa.id) for coa in self.coa_ids]})  
            self.write({'coa':True,'analysis_date':fields.Date.today(),'coa_ids':coas})
