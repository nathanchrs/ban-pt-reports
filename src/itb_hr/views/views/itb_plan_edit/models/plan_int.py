...
class Plan_Int(models.Model):
	...
	year = fields.Integer(default='2018')#date.today().year+1, required=True)
	...
	def _compute_name(self):
		if self.unit_id:
			self.name = str(self.unit_id.name + ' - 2018') #+ str(date.today().year+1))
	...
	def update_state_button(self):
		internal = self.env['itb.plan_int'].search([('state','=','draft')])
		internal.write({'state':'validate',})
		...
		if plan :
			...
			for w in p_int:
				if p_int:
					...
					for x in p_line_int:
						if x.id:
							plan_line = self.env['itb.plan_line']
							plan_line.create({
								'plan_id':plan.id,
								'name':x.name,
								'unit_id':w.unit_id.id,
								'program_id':x.program_id.id,
								'activity_id':x.activity_id.id,
								'subactivity_id':x.subactivity_id.id,
								'code':x.code,
								'barang':x.barang,
								'pegawai':x.pegawai,
								'jasa':x.jasa,
							...
							if p_line:
								plan_spending = self.env['itb.plan_spending']
								spend_int = self.env['itb.plan_spending_int'].search([('plan_line_id','=',x.id),('plan_int_id','=',w.id)])
								for aa in spend_int:
									cc = self.env['itb.plan_price'].search([('id','=',aa.price_id.id)])
									plan_spending.create({
										'plan_id':plan.id,
										'plan_line_id':p_line.id,
										'name':aa.name,
										'code':aa.code,
										'month':aa.month,
										'price_id':aa.price_id.id,
										'total':aa.total,
										'percent_budget':aa.percent_budget,
										'source':aa.source,
										'type':aa.type,
										...
										'volume':aa.volume,
										'day':aa.day,
										'unit_id':aa.unit_id.id,
										'activity_id':p_line.activity_id.id,
										'subactivity_id':p_line.subactivity_id.id,
										'program_id':p_line.program_id.id,
										'price':cc.amount,
								...
								target_int = self.env['itb.plan_target_int'].search([('plan_line_id','=',x.id),('plan_int_id','=',w.id)])
								target = self.env['itb.plan_target']
								for bb in target_int: 
									target.create({
										'plan_id':plan.id,
										'plan_line_id':p_line.id,
										'indicator_id':bb.indicator_id,
										'plan':bb.plan,
										'actual':bb.actual,
									})
								'''
	...
	def generateplan(self):
		...
		plan = self.env['itb.plan'].search([('year','=',thn)])
		if plan:
			raise models.ValidationError('plan year ' + str(thn) + ' already exist')
			#itb_plan = self.env['itb.plan'].search([('year','=',thn)])
    		#itb_plan.unlink()
		...
		plan = self.env['itb.plan'].search([('year','=',thn)])
		if plan :
			p_int = self.env['itb.plan_int'].search([('year','=',thn),('state','=','validate')], order='unit_id')
			budget_total = 0
			for w in p_int:
				if p_int:
					p_line_int = self.env['itb.plan_line_int'].search([('plan_int_id','=',w.id)])
					for x in p_line_int:
						if x.id:
							...
							self.env.cr.execute("""
								insert into itb_plan_line (subactivity_id, code, create_date, barang, write_uid, plan_id, 
										percent_performance, create_uid, percent_budget, pegawai, activity_id, 
										program_id, write_date, name, budget, jasa, modal, unit_id, is_project)
								SELECT a.subactivity_id, a.code,CURRENT_TIMESTAMP , a.barang, """ + str(self._uid) + """, """ + str(plan.id) + """, 
										a.percent_performance, """ + str(self._uid) + """, a.percent_budget, a.pegawai, a.activity_id, 
										a.program_id,CURRENT_TIMESTAMP, a.name, a.budget, a.jasa, a.modal,b.unit_id, a.is_project
								FROM public.itb_plan_line_int a, itb_plan_int b
								where a.plan_int_id=b.id and a.id='""" + str(x.id) + """'
								;
							""")
							...
							p_line = self.env['itb.plan_line'].search([('plan_id','=',plan.id)], order='id desc', limit=1)
							if p_line:
								p_sp = self.env['itb.plan_spending_int'].search([('plan_int_id','=',w.id),('plan_line_id','=',x.id)])
								if p_sp:
									for z in p_sp:
										'''
										plan_spending = self.env['itb.plan_spending']
										cc = self.env['itb.plan_price'].search([('id','=',z.price_id.id)])
										plan_spending.create({
											'plan_id':plan.id,
											'plan_line_id':p_line.id,
											'name':z.name,
											'code':z.code,
											'month':z.month,
											'price_id':z.price_id.id,
											'total':z.total,
											'percent_budget':z.percent_budget,
											'source':z.source,
											'type':z.type,
										...
										'''
										self.env.cr.execute("""
												INSERT INTO itb_plan_spending(plan_id, plan_line_id, price_id,
													code, create_date, month, total, create_uid, 
													percent_budget, source, type, available, used, paid, volume,
													write_date, write_uid, day, name, unit_id, activity_id,
													subactivity_id, program_id, price, standard) 
												select """ + str(plan.id) + """,""" + str(p_line.id) + """, a.price_id,
													a.code, CURRENT_TIMESTAMP, a.month,  a.total, """ + str(self._uid) + """,
													a.percent_budget, a.source, a.type, a.available, a.used, a.paid, a.volume,
													CURRENT_TIMESTAMP, """ + str(self._uid) + """, a.day, a.name, d.unit_id,
													b.activity_id, b.subactivity_id, b.program_id, c.amount, c.standard
												from itb_plan_spending_int a, itb_plan_line_int b, itb_plan_price c, itb_plan_int d
												where a.id = '""" + str(z.id) + """' and a.plan_line_id='""" + str(x.id) + """' 
													and a.plan_int_id='""" + str(w.id) + """' and a.plan_line_id = b.id and a.price_id=c.id
													and a.plan_int_id = d.id
											;
										""")
										
										budget=self.env['itb.plan_spending_int'].search([('plan_line_id','=',x.id),('plan_int_id','=',w.id),('id','=',z.id)])
										if budget:
											for rec in budget:
												p_spend = self.env['itb.plan_spending'].search([('plan_id','=',plan.id),('plan_line_id','=',p_line.id)], order='id desc', limit=1)
												budget_total = budget_total + rec.total
												self.env.cr.execute("""
														INSERT INTO itb_plan_allocation(
															create_uid, write_date, budget, write_uid, create_date, spending_id_int, 
													...
													;
												""")
								p_tg = self.env['itb.plan_target_int'].search([('plan_int_id','=',w.id),('plan_line_id','=',x.id)])
								if p_tg:
									for xx in p_tg:
										self.env.cr.execute("""
												INSERT INTO itb_plan_target(
												...
												select """ + str(self._uid) + """, """ + str(plan.id) + """, indicator_id, CURRENT_TIMESTAMP, plan, CURRENT_TIMESTAMP, 
													actual, """ + str(self._uid) + """, """ + str(p_line.id) + """ 
												from itb_plan_target_int
												where id = '""" + str(xx.id) + """' plan_line_id='""" + str(x.id) + """' and plan_int_id='""" + str(w.id) + """'
											;
										""")
